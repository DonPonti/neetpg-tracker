...
export default function SubjectTopicsDetail({
  ...
  // Render Category styling (Light theme friendly)
  ...
  return (
    <div id="syllabus-tracker-core" className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm relative overflow-hidden">
      
      {/* Visual Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <span className="text-xs font-semibold text-blue-600 uppercase tracking-widest flex items-center gap-1.5">
        ...
        {/* Total stats badge */}
        <div className="flex items-center gap-2 self-start md:self-auto text-xs font-bold px-3 py-1.5 bg-blue-50 border border-blue-105 text-blue-700 rounded-xl">
          <Sparkles size={14} className="text-blue-600 animate-pulse" /> Full National Curriculum Catalog
        </div>
      </div>

      {/* Filter Options */}
      <div id="filters-container" className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 pb-6 border-b border-slate-200">
        
        {/* Category Filter */}
        <div className="flex flex-col gap-1.5 col-span-1 md:col-span-2">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
            <Filter size={12} className="text-slate-450" /> Subject Category
          </label>
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
            <Search size={12} className="text-slate-455" /> Search Curriculum
          </label>
          <div className="relative">
            <input
              ...
              className="w-full text-xs pl-8 pr-3 py-2 border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-800 placeholder:text-slate-400 font-medium"
            />
            <Search size={14} className="absolute left-2.5 top-2.5 text-slate-450" />
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1">
            <AlertTriangle size={12} className="text-slate-450" /> Focus Rules
          </label>
          <div className="flex gap-2">
            <select
              value={priorityFilter}
              onChange={(e) => setPriorityFilter(e.target.value as any)}
              className="flex-1 text-xs px-2 py-1.5 border border-slate-200 bg-slate-50 rounded-xl focus:outline-none focus:ring-1 focus:ring-blue-500 text-slate-700 font-medium cursor-pointer"
            >
      ...
      <div id="subject-accordion-list" className="space-y-4">
        {processedSubjects.length === 0 ? (
          <div className="text-center py-12 bg-slate-50 rounded-2xl border border-dashed border-slate-200">
            ...
            <h3 className="text-sm font-bold text-slate-600">No Matched Syllabus</h3>
            <p className="text-xs text-slate-450 max-w-sm mx-auto mt-1 leading-normal">
          ...
          processedSubjects.map((sub) => {
            ...
            return (
              <div
                ...
                key={sub.id}
                className="group border border-slate-200 rounded-2xl overflow-hidden shadow-xs bg-slate-50/20 hover:border-slate-300 transition-all"
              >
                {/* Accordion Trigger Header */}
                <div
                  onClick={() => toggleSubject(sub.id)}
                  className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-50/40 hover:bg-slate-50 cursor-pointer select-none transition-colors border-b border-transparent group-hover:border-slate-200"
                >
                  <div className="flex flex-wrap items-center gap-2.5">
                    ...
                    <h3 className="text-sm font-black text-slate-800">{sub.name}</h3>
                    ...
                    {searchQuery.length > 0 && (
                      <span className="text-[10px] bg-sky-50 border border-sky-100 text-sky-700 font-extrabold px-2 py-0.5 rounded-full">
                  ...
                  <div className="flex items-center gap-4 mt-2.5 sm:mt-0">
                    ...
                    <div className="w-24 bg-slate-100 h-1.5 rounded-full overflow-hidden shrink-0 hidden md:block border border-slate-200">
                      <div
                    ...
                    <span className="text-xs font-black text-slate-700 pr-1">{completionRate}%</span>
                  </div>
                </div>
                ...
                {isExpanded && (
                  <div className="px-4 py-3 divide-y divide-slate-100 bg-white">
                    ...
                    <div className="flex items-center justify-between py-2 text-[10px] text-slate-400 font-extrabold mb-1">
                      ...
                      return (
                        <div
                          ...
                          className={`py-3 flex flex-col gap-2 relative transition-all ${
                            isTopicFullyDone ? 'bg-indigo-50/10' : ''
                          }`}
                        >
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            ...
                            <div className="flex items-start gap-2.5 flex-1 min-w-0">
                              ...
                              <button
                                ...
                                className={`p-1 rounded-lg hover:bg-slate-50 transition-colors shrink-0 mt-0.5 cursor-pointer ${
                                  topic.isDoubted
                              ...
                              <button
                                ...
                                className={`text-[9px] font-black uppercase px-2 py-0.5 rounded shrink-0 select-none mt-1 transition-all border cursor-pointer ${
                                  topic.priority === 'high'
                                    ...
                                    ? 'bg-orange-50 text-orange-650 border-orange-100 hover:bg-orange-100'
                            ...
                            <div className="flex items-center gap-1.5 bg-slate-50 p-1.5 rounded-xl w-full sm:w-auto self-stretch sm:self-auto justify-between sm:justify-start select-none border border-slate-200">
                              {stageKeys.map((stage) => {
                                ...
                                return (
                                  <label
                                    ...
                                    className={`group flex-1 sm:flex-none flex items-center justify-center p-0.5 flex-col aspect-square w-9 h-9 sm:w-8 sm:h-8 rounded-lg cursor-pointer border text-center font-extrabold transition-all duration-150 relative ${
                                      isChecked
                                        ? 'bg-blue-600 border-blue-500 text-white font-black shadow-xs'
                              ...
                              <button
                                ...
                                className={`p-2 sm:p-1.5 hover:bg-slate-200 rounded-lg transition-all shrink-0 ml-1 flex items-center justify-center cursor-pointer ${
                          ...
                          {editingNotesTopicId === topic.id && (
                            <div className="bg-slate-50 p-4 border border-slate-200 rounded-2xl mt-2 space-y-2">
                              <div className="flex justify-between items-center text-[10px] font-black text-amber-705 uppercase pb-1 border-b border-slate-200">
                                <span>Highlighted Formula Logs / Mnemonics</span>
                                <span className="text-slate-500 font-bold font-mono">Saves locally</span>
                              </div>
                              <textarea
                                value={notesBuffer}
                                onChange={(e) => setNotesBuffer(e.target.value)}
                                placeholder="Jot down high-yield diagnostics schema, guidelines or exceptions..."
                                className="w-full text-xs p-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-1 focus:ring-amber-500 h-20 resize-y font-medium text-slate-800"
                              />
                              <div className="flex gap-2 justify-end pt-1">
                                <button
                                  onClick={() => setEditingNotesTopicId(null)}
                                  className="text-[10px] px-2.5 py-1.5 text-slate-500 hover:bg-slate-100 rounded-lg font-bold"
                                >
                                  Close
                                </button>
                                <button
                                  onClick={() => handleNotesUpdateSave(sub.id, topic.id)}
                                  className="text-[10px] px-3.5 py-1.5 bg-amber-600 hover:bg-amber-505 text-white rounded-lg font-black shadow-sm cursor-pointer"
                                >
                                  Save Memo
                                </button>
                              </div>
                          ...
                          {editingNotesTopicId !== topic.id && topic.notesText && (
                            <div className="text-[10px] bg-slate-50 text-slate-600 border border-slate-200 p-2 rounded-xl mt-1 italic flex items-center justify-between">
                              ...
                              <button
                                ...
                                className="text-[10px] text-blue-650 font-bold hover:underline shrink-0"
                              >
                                Edit notes
                              </button>
                            </div>
